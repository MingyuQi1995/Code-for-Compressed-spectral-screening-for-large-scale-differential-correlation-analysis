...
evalres <- foreach(b = 1:B, .combine = rbind, .packages = c("irlba", "Matrix", "DGCA", "plyr", "ROCR")) %dopar% {
    ...
    # nv2 <- sample(1:g, n_nv2)
     ...
     screening <- fast.SS(X1= t(data1),X2=t(data2),rho=0.05,K=10,K.seq = TRUE,tune=TRUE)
     screening$tune.error
     khat <- which.min( screening$tune.error)
     
     realrank        <- rankMatrix(Real_D)
     kreal           <- as.numeric(realrank[[1]])
     k <-  khat
...
save(evalres, file = "res/evalresfullrankest.RData")
